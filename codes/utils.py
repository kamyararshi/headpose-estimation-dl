...
def draw_axis(img, yaw, pitch, roll, tdx=None, tdy=None, size = 100):
    """draws axis w.r.t the angles
    img: tensor of shape (1, C, H, W) or numpy image of hsape H,W,C
    """

    if len(img.shape) == 4 and isinstance(img, torch.Tensor):
        #Tensor to numpy shape H,W,C
        img = img.permute(0, 2, 3, 1).contiguous().cpu().squeeze(0).numpy()
